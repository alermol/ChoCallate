process CALL_GATK {
    ...
    'gatk HaplotypeCaller ${params.calling.gatk.extra_args} --input tmp/input.bam --reference tmp/ref_genome.fasta --min-base-quality-score "${params.calling.min_base_quality}" --output /dev/stdout --intervals {} --sequence-dictionary tmp/ref_genome.dict | bcftools reheader -s tmp/calling_chunks/sample_id.txt -f tmp/ref_genome.fasta.fai | bcftools filter --soft-filter 'LowQual' -e"QUAL<${params.calling.min_snp_qual}" -Ou | bcftools norm --check-ref x -m+ -Ou --fasta-ref tmp/ref_genome.fasta -o tmp/calling_chunks/{#}.bcf
