process CALL_VARSCAN {
    ...
    'samtools mpileup --fasta-ref tmp/ref_genome.fasta --positions {} tmp/input.bam | varscan mpileup2cns --output-vcf 1 --variants 1 | bcftools reheader -s tmp/calling_chunks/sample_id.txt -f tmp/ref_genome.fasta.fai | bcftools filter -e"GQ<${params.calling.min_snp_qual}" -Ou | bcftools norm --check-ref x -m+ -Ou --fasta-ref tmp/ref_genome.fasta -o tmp/calling_chunks/{#}.bcf
