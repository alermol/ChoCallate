process CALL_FREEBAYES {
    ...
    'freebayes --fasta-reference tmp/ref_genome.fasta --targets {} --dont-left-align-indels --ploidy "${params.ploidy}" --min-base-quality "${params.calling.min_base_quality}" ${params.calling.freebayes.extra_args} --bam tmp/input.bam | bcftools reheader -s tmp/calling_chunks/sample_id.txt -f tmp/ref_genome.fasta.fai | bcftools filter -e"QUAL<${params.calling.min_snp_qual}" -Ou | bcftools norm --check-ref x -m+ -Ou --fasta-ref tmp/ref_genome.fasta -o tmp/calling_chunks/{#}.bcf
