process CALL_MUTECT2 {
    ...
    'gatk Mutect2 ${params.calling.mutect2.extra_args} --input tmp/input.bam --reference tmp/ref_genome.fasta --min-base-quality-score "${params.calling.min_base_quality}" --output tmp/calling_chunks/{#}.vcf --intervals {} --sequence-dictionary tmp/ref_genome.dict;
    bcftools reheader -s tmp/calling_chunks/sample_id.txt -f tmp/ref_genome.fasta.fai tmp/calling_chunks/{#}.vcf | bcftools filter --soft-filter 'LowQual' -e"INFO/TLOD<${params.calling.min_snp_qual}" -Ou | bcftools norm --check-ref w -m+ -Ou --fasta-ref tmp/ref_genome.fasta -o tmp/calling_chunks/{#}.bcf;
